...
namespace CBDCHubContract.Services
{
    ...
    public class EthereumEventRetriever
    {
        ...
        public EthereumEventRetriever(ILogger<EthereumEventRetriever> logger, IOptions<EthereumConfig>  config)
        {
            _logger = logger;
            _config = config;
        ...
        {
            return _web3.Eth.GetEvent<T>(_config.Value.ContractAddress);
        }
        ...
        {
            //get the block
            var transaction = await _web3.Eth.Transactions.GetTransactionByHash.SendRequestAsync(_config.Value.ContractTransactionHash);
